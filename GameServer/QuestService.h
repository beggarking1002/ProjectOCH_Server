...
public:
	...
	void HandleAbandon(GameSessionRef session, const PlayerRef& player, const string& questId);
...
private:
	...
	void SendTrackerState(GameSessionRef session, const PlayerRef& player, bool success, const string& reason,
		const string& action = "open", int32 goldPenalty = 0, int32 famePenalty = 0) const;
};
