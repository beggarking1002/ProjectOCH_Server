...
현재 서버는 로컬 `127.0.0.1:7777`에서 최대 100 세션 설정으로 동작하는 개발 프로토타입이다. 계정 DB, 채팅 콘텐츠, 서비스 종료 처리, 부하 테스트는 아직 제품 범위에 포함하지 않았다.
...
| 사례 | 접근 | 적용 기능 |
| --- | --- | --- |
| 육각 거리 계산 | axial `(q, r)`를 cube `(q, r, -q-r)`로 확장하고 `(|dq| + |dr| + |ds|) / 2`를 사용 | 사거리, 인접, 이동, 광역 범위 |
...
특히 ZOC는 별도의 타일 목록을 매 프레임 보관하지 않는다. 각 적 Pawn 후보의 이동 시작 타일에 대해 **O(1) 육각 거리 판정 + 6방향 내적 비교**를 수행하고, 클래스 프로필의 사거리·전방 폭·반응 횟수·트리거를 결합해 기회 공격 여부를 결정한다. 즉 맵 타일 수에 비례하는 탐색 없이, 전투 참여 Pawn 수만큼만 판정한다.
...
현재 최신 검증 기준: `GameServer Debug build — warning 0, error 0`.
...
- Beige Ice / Beige Fire / Suen Axe의 주요 전투 규칙과 전용 Pawn 클래스
- Alen Spear의 전용 Pawn 클래스, 2칸 직선 공격, 밀쳐내기 충돌, 파수꾼, 사기·이동 버프
- 공용 효과 실행기, 상태·자원·보호막·타일 시스템
- 근접 반격, Suen Axe·Alen Spear ZOC 프로필, 사기·어지러움 기반
- Zillian Longbow의 단일 피해 중심 CSV 행
