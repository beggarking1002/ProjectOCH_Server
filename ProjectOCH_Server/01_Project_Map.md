...
- `ServerPacketHandler.h`: 클라이언트가 서버로 보내는 C_ 패킷 생성 helper.
- `*.pb.h/.cc`: protobuf 생성물.
...
`Common\protoc-21.12-win64\include`는 protoc 배포판 include다.

## Tools

`Tools\PacketGenerator`는 packet handler 생성기다.

- `PacketGenerator.py`: cli entry. 기본 path가 `Common\protoc-21.12-win64\bin\Protocol.proto`를 가리킨다.
...
- `GenPackets.exe`: Python 생성기를 exe로 만든 것으로 보인다.
